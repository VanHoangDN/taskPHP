...
function get_all_tasks($conn){
    ...
    $tasks = $stmt->rowCount() > 0 ? $stmt->fetchAll() : [];
...
function get_all_tasks_overdue($conn){
    $sql = "SELECT * FROM tasks WHERE due_date < CURDATE() AND due_date IS NOT NULL AND status != 'completed' ORDER BY id DESC";
    $stmt = $conn->prepare($sql);
    $stmt->execute([]);
...
function count_tasks_overdue($conn){
    $sql = "SELECT id FROM tasks WHERE due_date < CURDATE() AND due_date IS NOT NULL AND status != 'completed'";
    $stmt = $conn->prepare($sql);
    $stmt->execute([]);
...
function get_task_by_id($conn, $id){
    ...
    $task = $stmt->rowCount() > 0 ? $stmt->fetch() : null;
...
function count_my_tasks_overdue($conn, $id){
    $sql = "SELECT id FROM tasks WHERE due_date < CURDATE() AND due_date IS NOT NULL AND status != 'completed' AND assigned_to=?";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$id]);
